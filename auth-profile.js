...
async function handleAuthSubmit(e) {
  ...
  try {
    ...
    // Восстанавливаем состояние кнопки, чтобы форма никогда не оставалась заблокированной
    btn.disabled = false;
    btn.innerText = (currentAuthMode === 'login') ? 'Войти' : 'Создать аккаунт';
  }
}

// Сброс полей и кнопки формы авторизации
...
function logoutUser() {
  showDialog('Выход', 'Точно выйти из аккаунта?', true, async () => {
    try {
      ...
      // Сбрасываем кэш до чистого безопасного состояния
...
// Открытие модального окна удаления аккаунта с бесшовной реаутентификацией
...
// Вспомогательный метод для полной очистки данных пользователя и удаления учетной записи
...
// Удаление аккаунта по паролю (с мгновенной верификацией пароля без ручного выхода)
async function submitDeleteAccountWithPassword() {
  ...
  try {
    // 1. Сначала подтверждаем пароль через официальный механизм reauthenticateWithCredential
    const credential = firebase.auth.EmailAuthProvider.credential(user.email, password);
    await user.reauthenticateWithCredential(credential);

    // 2. Реаутентификация успешна — теперь безопасно стираем данные и удаляем аккаунт
...
// Удаление аккаунта через Google (с быстрым всплывающим окном reauthenticateWithPopup)
async function submitDeleteAccountWithGoogle() {
  ...
  try {
    ...
    // Реаутентификация Google успешна — стираем данные
    await purgeUserDataAndAuthUser(user);
  } catch (err) {
...
function openProfileModal() {
  ...
  // Обновляем видимость пункта установки приложения в профиле
